...
uses
  System.Types, System.UITypes, System.Generics.Collections, FMX.Types, FMX.TextLayout, FMX.FontGlyphs, FMX.Graphics, PersianTool, System.StrUtils;
...
begin
  ...
  LastText := '';
  NewText := AText;
  LArrWords := NewText.Split([SpaceChar]);
  for J := Low(LArrWords) to High(LArrWords) do
  begin
    NewText := LArrWords[J];
    if (NewText <> '') and (TFarsi.IsFarsiChar(NewText[1])) then
    begin
      NewText := TFarsi.ConvertWord2(NewText);
      NewText := ReverseString(NewText);
      LastText := NewText + SpaceChar + LastText;
    end
    else
    begin
      LastText := LastText + SpaceChar + NewText;
    end;
  end;
  I := 0;
  while I < LastText.Length do
  begin
    FChars.Add(LastText[I].ToUCS4Char);
    CharLength := 1;
//    if (TFarsi.IsFarsiChar(LastText[I])) or (TFarsi.IsSpecialChar(LastText[I])) then
//    begin
//      FChars.Add(LastText[I].ToUCS4Char);
//      CharLength := 1;
//    end
//    else
//    begin
//      CharID := System.Char.ConvertToUtf32(LastText, I + FStartIndex, CharLength);
//      FChars.Add(System.Char.ConvertToUtf32(LastText, I + FStartIndex, CharLength));
//    end;
    Inc(I, CharLength);
  end;
  //---------------------------
...
//  while I < FLength do
//  begin
//    CharID := System.Char.ConvertToUtf32(NewText, I + FStartIndex, CharLength);
//    FChars.Add(System.Char.ConvertToUtf32(NewText, I + FStartIndex, CharLength));
//    Inc(I, CharLength);
//  end;
end;
